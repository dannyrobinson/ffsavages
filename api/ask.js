...
export async function POST(req) {
  ...
  try {
    ...
    const [sweep, advice, subs] = await Promise.all([sweepFor(), kvGet("advice").catch(() => null), kvGet("subs").catch(() => null)]);
    const system = `${RULES}\n\n${contextText(sweep, { news: String(news || "").slice(0, 20000), prev: advice, reason: "Danny is asking a question in the app", subs })}` +
      (advice ? `\n\nYour latest full advice (${advice.when}): ${advice.headline} ${advice.summary} Lineup changes: ${(advice.lineup || []).map(l => `${l.slot}: ${l.start} over ${l.sit || "empty"}`).join("; ") || "none"}. IR moves: ${(advice.ir || []).map(i => `${i.player} ${i.move}`).join("; ") || "none"}. Auto-subs to set: ${(advice.subs || []).map(s => `${s.sub} for ${s.starter}`).join("; ") || "none"}. Adds: ${(advice.adds || []).map(w => `${w.add} for ${w.drop || "the spot freed by IR"} (${w.how}${w.how === "waiver claim" && w.bid ? `, bid ${w.bid}${w.backup ? ", backup claim" : ""}` : ""})`).join("; ") || "none"}.` : "") +
