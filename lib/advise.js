// The advisor: fresh Sleeper sweep + web search -> Claude -> structured advice, stored in kv. Sleeper's projected
// POINTS are deliberately withheld from the context (Danny, Sept 12: he sees them himself); volume, lines and news drive it.
// and pushed to Danny's phone when something needs doing.
// Since Sept 13 the scheduled advisor is a Claude cloud routine on Danny's subscription (no API spend): it reads
// the prompt from api/context (or data/context.json, which the GitHub Action commits before each run), researches,
// and commits data/advice.json; the 15-minute checker ingests that file (ingestAdvice), stores it and pushes its
// alerts. runAdvisor (the Anthropic API) remains only for the app's on-demand Re-check, under ADVISE_DAILY_CAP.
...
export const ADVISE_CAP = Math.max(1, +(process.env.ADVISE_DAILY_CAP || 12));   // runs a day, every trigger included
export const SEARCHES = Math.max(0, +(process.env.ADVISE_SEARCHES || 5));       // web searches per run
const ADVICE_URL = "https://raw.githubusercontent.com/dannyrobinson/ffsavages/main/data/advice.json";

// Research guidance shared by the API advisor (web_search tool) and the cloud routine (WebSearch).
export const RESEARCH = `Research order, one focused search each, before you write anything: (1) Danny's amber and red players, and for a starter you intend to pair with an auto-sub the kickoff times of his game and the sub's; (2) every player you intend to name in a lineup change or an add/drop, BOTH sides of each swap, searching "<name> <NFL team> news" for role, injury, suspension or exempt-list status and the status of whoever is ahead of him on the depth chart; (3) any bench player whose room shows him as his team's #1, to confirm the role is real and not a committee; (4) every player another team dropped in the last 4 days who is still unowned: why he was dropped and whether he beats Danny's worst bench player (say so in "watch" when he does not); (5) when two of Danny's QBs share a bye in the next 5 weeks, the best free-agent QB whose bye differs; (6) only when two candidates for one lineup slot are close on role, usage and game environment, the opponent defence as the tiebreaker: its pressure rate and what it allows to that position, from a source dated this season. Kickoff times, the Vegas lines and the weather are already in the data; never spend a search on them. Searches are scarce: spend them on the decisions that change what Danny does today, and skip the rest. Output discipline: write nothing outside the JSON object, no notes before, between or after searches; keep every string to one short sentence and the whole reply compact. If a search contradicts the room shown in the data, prefer the newer, dated source and say so. Mention the date of the news you rely on. Do not include URLs. Only give advice you have checked this way; if you ran out of searches before checking a swap, leave that swap out and put the player in "watch" instead.`;

/** The prompt, in two parts, for whoever runs the advisor: the playbook, schema and research order, then the situation. */
export function promptFor(sweep, opts = {}) {
  return { system: `${RULES}\n\n${SCHEMA}\n\n${RESEARCH}`, user: contextText(sweep, opts) + "\n\nProduce the JSON now." };
}

/** Danny's stored state that the prompt and the store need. Missing kv is treated as empty. */
export async function stateFor() {
  const prev = await kvGet("advice").catch(() => null);
  const notified = (await kvGet("notified").catch(() => null)) || {};
  const subs = await kvGet("subs").catch(() => null);
  const pending = ((await kvGet("pending_reasons").catch(() => null)) || {}).items || [];
  return { prev, notified, subs, pending };
}

/** What the checker saw since the last advisor run (kv pending_reasons): appended by lib/check.js, cleared here. */
export async function addPendingReasons(items) {
  const cur = (await kvGet("pending_reasons").catch(() => null)) || { items: [] };
  const seen = new Set(cur.items);
  for (const r of items) if (r && !seen.has(r)) { seen.add(r); cur.items.push(r); }
  cur.items = cur.items.slice(-30); cur.ts = Date.now();
  await kvSet("pending_reasons", cur);
}
...
{"headline": "one sentence: the single most important thing Danny should do right now, with the deadline (kickoff in Pacific time) when there is one",
 ...
 "lineup": [{"slot": "FLEX", "start": "player to put in", "sit": "player he comes out for, or null if the slot is empty", "why": "one sentence with the fact that decides it (a role, a usage figure, a line, a status), never a projected-point figure", "evidence": "the dated facts you verified for BOTH players (role, injury, who is ahead of him)", "urgency": "high|medium|low"}],
...
const row = r => `${r.pos} ${r.name} (${r.team})${r.flag !== "green" ? ` [${r.flag.toUpperCase()}: ${r.why}]` : ""}${r.vol ? ` · volume ${r.vol}` : ""}${r.rank ? ` · Sleeper ${r.pos} rank ${r.rank}` : ""} · ${r.game || "?"}${r.locked ? " · LOCKED" : ""}${r.line ? ` · ${r.line}` : ""}${r.env ? ` · ${r.env}` : ""}${r.bye ? ` · bye ${r.bye}` : ""}${av(r)}${r.room ? ` · room: ${r.room}` : ""}`;
...
export function contextText(sweep, { news = "", prev = null, notified = {}, reason = "", subs = null } = {}) {
  ...
  L.push(`Danny's roster (slot · player · Sleeper flag · projected volume: targets, carries, pass attempts (Sleeper's projected POINTS are deliberately withheld: Danny sees those himself and does not want advice built on them) · game with the kickoff in Pacific time · the Vegas line from his team's side with both implied totals · the weather at kickoff):`);
  for (const r of sweep.roster) L.push(`  ${r.slot.padEnd(5)} ${row(r)}`);
  const lu = sweep.lineup;
  if (lu) {
    L.push(`Current starting lineup: ` + lu.current.map(c => `${c.slot} ${c.name || "EMPTY"}`).join(", "));
  }
  ...
  if (as && as.max) {
    ...
    if ((as.at_risk || []).length) L.push(`Starters in doubt and the bench players who could be their auto-sub (kickoffs in Pacific time; ${as.any_start_time ? "any kickoff is fine here, so rank the subs by role and matchup; the pair has to be set before the earlier of the two kickoffs" : "the sub must kick off at the same time or later than the starter"}):` + as.at_risk.map(a => `${a.starter} (${a.slot}, ${a.status}, ${a.game}): ${a.subs.length ? a.subs.map(s => `${s.name} (${s.pos}, ${s.game}, ${s.timing})`).join("; ") : "no eligible bench player"}`).join(" | "));
  ...
  if (d) L.push(`DEF (Danny streams by matchup; the opponent's implied total is the first filter): he has ${d.mine ? `${d.mine.name} (${d.mine.game}${oi(d.mine)}; week ${sweep.next_week}: ${d.mine.next.game}${oi(d.mine.next)}${d.mine.bye ? `; bye ${d.mine.bye}` : ""})` : "no DEF"}. Best free-agent DEF this week: ${d.fa_this.map(x => `${x.name} (${x.game}${oi(x)}${x.waivers ? ", on waivers" : ""})`).join("; ") || "none"}. Best free-agent DEF for week ${sweep.next_week}: ${d.fa_next.map(x => `${x.name} (${x.game}${oi(x)})`).join("; ") || "no lines yet"}.`);
  ...
  if (o && o.team) L.push(`This week's opponent: ${o.team} (${o.record}), starters: ` + o.starters.map(row).join("; "));
  ...
  if (sweep.fa_top) {
    L.push(`Free agents in THIS league worth a look this week (Sleeper's top few at each position; judge them by role, usage and matchup):`);
    for (const [pos, rows] of Object.entries(sweep.fa_top)) if (rows.length) L.push(`  ${pos}: ` + rows.map(r => `${r.name} (${r.team}, ${r.game}${r.flag !== "green" ? `, ${r.flag}: ${r.why}` : ""}${av(r)}${r.room ? `; room: ${r.room}` : ""})`).join("; "));
  }
  if (sweep.trending.length) L.push(`Trending adds across Sleeper in the last 48h that are free agents here: ` + sweep.trending.map(r => `${r.pos} ${r.name} (${r.team}, ${r.adds} adds${r.flag !== "green" ? `, ${r.flag}: ${r.why}` : ""}${av(r)}${r.room ? `; room: ${r.room}` : ""})`).join("; "));
  const recent = sweep.transactions.filter(t => t.ts && Date.now() - t.ts < 4 * 24 * H);
  if (recent.length) L.push(`League transactions in the last 4 days (newest first; a bid is what the winner paid): ` + recent.map(t => `${fmt(t.ts)} ${t.team} ${t.type}${t.bid != null ? ` (bid ${t.bid})` : ""}${t.status !== "complete" ? ` [${t.status}${t.note ? `: ${t.note}` : ""}]` : ""}${(t.faab_moved || []).length ? ` (FAAB moved: ${t.faab_moved.map(m => `${m.amount} ${m.from} to ${m.to}`).join(", ")})` : ""}: added ${t.adds.join("/") || "-"}, dropped ${t.dropped.length ? t.dropped.map(d => `${d.name} (${d.pos} ${d.team}${d.fa ? `, STILL UNOWNED${av(d)}` : ", since claimed"})`).join("/") : (t.drops.join("/") || "-")}`).join("; "));
  L.push(`QBs rostered per team (2 = a buyer for a spare QB): ` + sweep.qb_depth.map(q => `${q.team} ${q.n}`).join(", "));
  if ((sweep.teams || []).length) L.push(`Every team's roster, for trades and for reading who is thin where (counts are the whole roster; BN is their bench, IR their reserve). Danny's own roster is above in full:\n` +
    sweep.teams.map(t => `- ${t.team}${t.mine ? " (Danny)" : ""} ${t.record}, ${t.pts_for} PF, ${t.faab_left} FAAB: ` +
      Object.entries(t.counts).sort().map(([k, v]) => `${k}${v}`).join(" ") +
      (t.bench.length ? ` | BN ${t.bench.join(", ")}` : "") + (t.ir.length ? ` | IR ${t.ir.join(", ")}` : "")).join("\n"));
...
export async function runAdvisor({ trigger = "schedule", reason = "", news = "", push = true, dry = false, sweep = null, searches = SEARCHES, subs = null } = {}) {
  sweep = sweep || await buildSweep({ force: true });
  const prev = await kvGet("advice").catch(() => null);
  const pending = ((await kvGet("pending_reasons").catch(() => null)) || {}).items || [];
  if (pending.length) reason = `${reason || "scheduled check"}. Since the last read the checker also saw: ${pending.join("; ")}`;
  const notified = (await kvGet("notified").catch(() => null)) || {};
  ...
  const call = withSearch => callClaude({
    ...
    system: `${RULES}\n\n${SCHEMA}` + (withSearch ? `\n\nYou have a web_search tool. ${RESEARCH}` : ""),
  ...
  const meta = { model: res.model, searches: res.searches, fallback: res.fallback || null, usage: res.usage && { in: res.usage.input_tokens, out: res.usage.output_tokens, cached: res.usage.cache_read_input_tokens, cache_write: res.usage.cache_creation_input_tokens } };
  return storeAdvice(data, { sweep, trigger, reason, notified, subs, pending, push, dry, meta });
}

/** Turn a reply in the schema shape into the stored advice record, push its alerts (deduplicated by key) and clear the parked reasons. */
export async function storeAdvice(data, { sweep, trigger = "routine", reason = "", notified = {}, subs = null, pending = [], push = true, dry = false, meta = {} }) {
  ...
  if (!dry) {
    ...
    if (pending.length) await kvSet("pending_reasons", { items: [], ts: Date.now() });
    await kvSet("notified", notified);
  }
  return { ok: true, dry, advice, sent };
}

/** The cloud routine commits data/advice.json; when the file is newer than the last one ingested, store it and push its alerts. */
export async function ingestAdvice({ sweep }) {
  let r, data;
  try { r = await fetch(`${ADVICE_URL}?t=${Date.now()}`, { headers: { "cache-control": "no-cache" } }); }
  catch (e) { return { ok: false, error: `GitHub unreachable: ${e.message || e}` }; }
  if (r.status === 404) return { ok: true, new: false, note: "no data/advice.json yet" };
  if (!r.ok) return { ok: false, error: `GitHub ${r.status}` };
  try { data = await r.json(); } catch { return { ok: false, error: "data/advice.json is not valid JSON" }; }
  const ts = Number(data.ts) || 0;
  if (!ts || !data.headline) return { ok: false, error: "data/advice.json has no ts or headline" };
  const last = await kvGet("advice_ingested").catch(() => null);
  if (last && last.ts >= ts) return { ok: true, new: false };
  if (Date.now() - ts > 3 * 24 * H) return { ok: true, new: false, note: "file is older than 3 days; ignored" };
  const st = await stateFor();
  const res = await storeAdvice(data, { sweep, trigger: "routine", reason: String(data.reason || "cloud routine"), notified: st.notified, subs: st.subs, pending: st.pending, push: true, meta: { model: String(data.model || "cloud routine"), written: ts } });
  await kvSet("advice_ingested", { ts, at: Date.now() });
  return { ...res, new: true };
}
