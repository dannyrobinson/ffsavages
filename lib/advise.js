...
{"headline": "one sentence: the single most important thing Danny should do right now, with the deadline (kickoff in Pacific time) when there is one",
 ...
 "adds": [{"add": "name", "pos": "RB", "team": "NFL team", "drop": "name on Danny's roster, or null when an IR move frees the spot first", "how": "add now|waiver claim", "why": "one sentence", "evidence": "the dated facts you verified for the add AND the drop, including the status of whoever is ahead of each on his NFL depth chart", "urgency": "high|medium|low"}],
...
export function contextText(sweep, { news = "", prev = null, notified = {}, reason = "" } = {}) {
  const L = [];
  L.push(`Now: ${nowPT()}. NFL week ${sweep.week}. Danny's record ${sweep.record}. Waiver priority ${sweep.waiver_position || "?"} of 12 (no bidding in this league). Sleeper data as of ${sweep.generated}.`);
  ...
  if (recent.length) L.push(`League transactions in the last 4 days (newest first): ` + recent.map(t => `${fmt(t.ts)} ${t.team} ${t.type}: added ${t.adds.join("/") || "-"}, dropped ${t.dropped.length ? t.dropped.map(d => `${d.name} (${d.pos} ${d.team}, proj ${d.proj}${d.fa ? ", STILL A FREE AGENT" : ", since claimed"})`).join("/") : (t.drops.join("/") || "-")}`).join("; "));
