...
{"headline": "one sentence: the single most important thing Danny should do right now, with the deadline (kickoff in Pacific time) when there is one",
 ...
 "subs": [{"starter": "Danny's starter who might not play", "sub": "bench player to set as his Sleeper AutoSub", "slot": "the starter's slot", "why": "one sentence: the starter's status and when the sub kicks off relative to him", "urgency": "high|medium|low"}],
...
export function contextText(sweep, { news = "", prev = null, notified = {}, reason = "", subs = null } = {}) {
  ...
  if (as && as.max) {
    L.push(`Sleeper AutoSubs are ON in this league: up to ${as.max} per team per week. Danny sets one in the Sleeper app (Swap Player, then "Set an AutoSub"): a bench player allowed in the starter's slot who is swapped in automatically if the starter is inactive at kickoff. Both players in a pair lock the moment EITHER game kicks off, so a sub whose game starts before the starter's only covers a downgrade announced before the sub's own kickoff${as.any_start_time ? " (the league allows such pairings, but they are weak cover)" : " (the league requires the sub to play at the same time or later)"}; if the starter plays, the sub is ${as.sub_freed_if_starter_plays ? "released and can be used later in the week" : "still locked for the week"}. Sleeper's API does not reveal which subs Danny has set; his own note is the record.`);
    ...
    if ((as.at_risk || []).length) L.push(`Starters in doubt and the bench players who could be their auto-sub (kickoffs in Pacific time; a sub must kick off at the same time or later than the starter to be real cover): ` + as.at_risk.map(a => `${a.starter} (${a.slot}, ${a.status}, ${a.game}): ${a.subs.length ? a.subs.map(s => `${s.name} (${s.pos}, proj ${s.proj}, ${s.game}, ${s.timing})`).join("; ") : "no eligible bench player"}`).join(" | "));
