...
export function contextText(sweep, { news = "", prev = null, notified = {}, reason = "", subs = null } = {}) {
  ...
  if (as && as.max) {
    ...
    if ((as.at_risk || []).length) L.push(`Starters in doubt and the bench players who could be their auto-sub (game days from Sleeper; the data has no kickoff windows, so check them before pairing): ` + as.at_risk.map(a => `${a.starter} (${a.slot}, ${a.status}, ${a.game}): ${a.subs.length ? a.subs.map(s => `${s.name} (${s.pos}, proj ${s.proj}, ${s.game}, ${s.timing})`).join("; ") : "no eligible bench player"}`).join(" | "));
