...
const row = r => `${r.pos} ${r.name} (${r.team})${r.flag !== "green" ? ` [${r.flag.toUpperCase()}: ${r.why}]` : ""} · proj ${r.proj ?? "?"} · ${r.game || "?"}${r.locked ? " · LOCKED" : ""}${av(r)}${r.room ? ` · room: ${r.room}` : ""}`;
