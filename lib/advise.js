...
const row = r => `${r.pos} ${r.name} (${r.team})${r.flag !== "green" ? ` [${r.flag.toUpperCase()}: ${r.why}]` : ""} · proj ${r.proj ?? "?"}${r.vol ? ` (${r.vol})` : ""}${r.rank ? ` · Sleeper ${r.pos} rank ${r.rank}` : ""} · ${r.game || "?"}${r.locked ? " · LOCKED" : ""}${r.line ? ` · ${r.line}` : ""}${r.env ? ` · ${r.env}` : ""}${r.bye ? ` · bye ${r.bye}` : ""}${av(r)}${r.room ? ` · room: ${r.room}` : ""}`;
