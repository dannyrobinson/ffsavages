...
export async function runAdvisor({ trigger = "schedule", reason = "", news = "", push = true, dry = false, sweep = null, searches = 8 } = {}) {
  ...
  catch (e) { throw { code: "invalid_json", message: `Claude's reply wasn't the JSON shape asked for (stop_reason ${res.stop_reason}, ${res.searches} searches, ${res.text.length} chars): ${String(res.text).slice(0, 200)}`, status: 502 }; }
