...
export function changeReasons(prev, cur, sweep, alerts) {
  ...
  for (const t of sweep.transactions) {
    ...
    R.push(`League move: ${t.team} ${t.type}${t.bid != null ? ` $${t.bid}` : ""} added ${t.adds.join(", ") || "-"}${drops ? `, dropped ${drops}` : ""}`);
  }
  return R;
}
...
export async function runCheck({ dry = false } = {}) {
  ...
  if (!dry) {
    ...
    await kvSet("last_check", {
      ...
      advisor: advisor ? (advisor.ok ? { headline: advisor.advice.headline, pushed: advisor.sent.map(s => s.key) } : { error: advisor.error }) : null,
    });
  }
