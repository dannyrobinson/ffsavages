...
// free-agent QB) record it. Only an URGENT change (a red flag on one of his starters, a starting QB hitting
// waivers) runs the advisor, at most every 90 minutes and under the shared daily cap; everything else is parked
// in kv pending_reasons for the next scheduled run (Danny called uncle on the spend, Sept 13). A red flag on one
// of his starters is pushed immediately as well, so a Claude outage can't hide it.
import { buildSweep } from "./sleeper.js";
import { kvGet, kvSet } from "./db.js";
import { sendAll } from "./push.js";
import { runAdvisor, addPendingReasons, ADVISE_CAP } from "./advise.js";
import { underCap } from "./http.js";

const MIN_GAP = 90 * 60 * 1000;   // between change-triggered advisor runs
...
export async function runCheck({ dry = false } = {}) {
  ...
  if (!dry) {
    ...
    if (reasons.length) {
      const urgent = alerts.some(a => a.critical || a.urgent);
      const last = urgent && await kvGet("advice").catch(() => null);
      const soon = last && last.ts && Date.now() - last.ts < MIN_GAP;
      if (urgent && !soon && await underCap("advise", ADVISE_CAP)) {
        ...
        await addPendingReasons(reasons);
        advisor = { ok: false, error: urgent ? (soon ? "urgent, but the advisor ran under 90 min ago; parked" : "daily advisor cap reached; parked") : "not urgent; parked for the next scheduled run" };
      }
    }
