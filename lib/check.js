// The alert checker: rebuild the sweep, diff Danny's roster against the last run, push what changed.
...
function snapshot(sweep) {
  ...
  return { week: sweep.week, ts: sweep.generated_ms, players, fa_qbs: sweep.fa_qbs.map(q => q.id) };
}
  ...
  for (const [pid, p] of Object.entries(cur.players)) {
    ...
    if (p.flag !== was.flag) {
      if (p.flag === "red") alerts.push({
        title: `${EMOJI.red} ${p.name}: ${label}`, tag: `rsv-${pid}`,
...
export async function runCheck({ dry = false } = {}) {
  ...
  const sent = [];
  if (!dry) {
    for (const a of alerts) sent.push({ title: a.title, results: await sendAll({ ...a, url: "./" }) });
    delete cur.fa_qb_rows;
    await kvSet("roster_state", cur);
    await kvSet("last_check", {
      ts: Date.now(), generated: sweep.generated, week: sweep.week, first_run: !prev,
      alerts: alerts.map(a => a.title), delivered: sent.reduce((n, s) => n + s.results.filter(r => r.ok).length, 0),
    });
  }
  return { ok: true, dry, first_run: !prev, week: sweep.week, generated: sweep.generated, alerts, sent };
}
