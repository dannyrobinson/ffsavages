...
export function diffAlerts(prev, cur) {
  ...
  for (const pid of cur.fa_qbs) {
    ...
    alerts.push({
      ...
      body: "Starts for his NFL team and nobody in the league has him. Superflex priority: add him now if he is a free agent, or get a claim in before Wednesday.",
    });
  }
...
export function changeReasons(prev, cur, sweep, alerts) {
  ...
  for (const t of sweep.transactions) {
    ...
    R.push(`League move: ${t.team} ${t.type} added ${t.adds.join(", ") || "-"}${drops ? `, dropped ${drops}` : ""}`);
  }
  return R;
}
