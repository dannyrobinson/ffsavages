...
// free-agent QB) push it to Danny's phone as it is and park the reasons in kv pending_reasons for the next advisor
// run. The advisor itself is the Claude cloud routine (Danny's subscription, no API spend since Sept 13): it commits
// data/advice.json and this checker ingests the file (lib/advise.js ingestAdvice) and pushes its alerts.
import { buildSweep } from "./sleeper.js";
import { kvGet, kvSet } from "./db.js";
import { sendAll } from "./push.js";
import { addPendingReasons, ingestAdvice } from "./advise.js";
...
export function diffAlerts(prev, cur) {
  ...
  for (const pid of cur.fa_qbs) {
    ...
    alerts.push({
      title: `QB on waivers: ${q ? q.name : pid}${q ? ` (${q.team})` : ""}`, tag: `rsv-faqb-${pid}`, urgent: true,
      body: "Starts for his NFL team and nobody in the league has him. Superflex priority: add him now if he is a free agent, or get a claim in with a real bid before he clears.",
    });
  }
...
export function changeReasons(prev, cur, sweep, alerts) {
  ...
  for (const t of sweep.transactions) {
    ...
    if (t.team === MY_TEAM) {   // his own claim landed or failed: the advisor should know what to do next
      if (t.type === "waiver") R.push(`Danny's waiver claim for ${t.adds.join(", ") || "-"} ${t.status}${t.bid != null ? ` (bid ${t.bid})` : ""}${t.note ? `: ${t.note}` : ""}`);
      continue;
    }
    if (t.status !== "complete") continue;
    const drops = t.dropped.map(d => `${d.name} (${d.pos} ${d.team}${d.fa ? `, now unowned${d.waivers ? ` (${d.waivers})` : ", a free agent"}` : ""})`).join(", ");
    R.push(`League move: ${t.team} ${t.type}${t.bid != null ? ` (bid ${t.bid})` : ""} added ${t.adds.join(", ") || "-"}${drops ? `, dropped ${drops}` : ""}`);
  }
  return R;
}
...
export async function runCheck({ dry = false } = {}) {
  ...
  if (!dry) {
    // critical first (a red flag on a starter), then the rest; the tag replaces an earlier alert about the same player
    for (const a of [...alerts.filter(a => a.critical), ...alerts.filter(a => !a.critical)]) sent.push({ title: a.title, results: await sendAll({ ...a, url: "./" }) });
    if (reasons.length) await addPendingReasons(reasons);
    try { advisor = await ingestAdvice({ sweep }); } catch (e) { advisor = { ok: false, error: String(e.message || e) }; }
    ...
    await kvSet("last_check", {
      ...
      advisor: advisor && advisor.new ? { headline: advisor.advice.headline, pushed: advisor.sent.map(s => `${s.key} (${s.delivered} delivered)`) } : advisor && advisor.error ? { error: advisor.error } : null,
    });
  }
  return { ok: true, dry, first_run: !prev, week: sweep.week, generated: sweep.generated, alerts, reasons, sent, advisor: advisor && (advisor.new ? { headline: advisor.advice.headline, pushed: advisor.sent } : advisor) };
}
