...
export async function buildSweep({ force = false } = {}) {
  ...
  const envErrors = [];   // a failed ESPN/Open-Meteo fetch leaves the environment fields null and is reported as env_error
  const envOf = w => gamesFor(st.season, w).catch(e => { envErrors.push(`week ${w}: ${e && e.message || e}`); console.error("games", w, e); return {}; });
