...
import { playerNews } from "./news.js";
...
export async function players(force = false) {
  ...
  for (const [k, v] of Object.entries(raw)) {
    ...
    map[k] = {
      ...
      search_rank: v.search_rank || null, espn_id: v.espn_id || null,
    };
  }
...
export async function buildSweep({ force = false } = {}) {
  ...
  // headlines for Danny's own players: Sleeper gives only a date, so the stories come from ESPN (lib/news.js).
  // Best effort — a failure leaves the list empty and `news_error` says why. Each roster row carries its own.
  const { stories, error: news_error } = await playerNews(roster.map(r => ({ id: r.id, name: r.name, team: r.team, espn_id: (P[r.id] || {}).espn_id })));
  const storiesFor = {};
  for (const st of stories) for (const pl of st.players) (storiesFor[pl.id] = storiesFor[pl.id] || []).push(st);
  for (const r of roster) r.stories = (storiesFor[r.id] || []).slice(0, 4).map(({ players, ...rest }) => rest);

  // every team's full roster (starters by slot, bench, IR) with records, points and FAAB — the trade view:
  // who is thin where, who is desperate, who can pay. Player strings, not rows; the detail lives in `roster`.
  const pStr = pid => { const p = P[pid] || {}; return `${p.name || pid} ${p.pos || "?"} ${p.team || "FA"}${p.injury_status ? ` (${p.injury_status})` : ""}`; };
  const weekPts = Object.fromEntries((Array.isArray(mus) ? mus : []).map(m => [m.roster_id, m.points]));
  const teams = rosters.map(r => {
    const s = r.settings || {}, res = r.reserve || [], st8 = (r.starters || []).filter(x => x), all = r.players || [];
    const counts = {};
    for (const pid of all) { const pos = (P[pid] || {}).pos; if (pos) counts[pos] = (counts[pos] || 0) + 1; }
    return {
      team: teamOf[r.roster_id], roster_id: r.roster_id, mine: r.owner_id === ME,
      record: `${s.wins || 0}-${s.losses || 0}${s.ties ? `-${s.ties}` : ""}`,
      pts_for: Number(`${s.fpts || 0}.${s.fpts_decimal || 0}`), pts_week: weekPts[r.roster_id] ?? null,
      faab_left: budget - (s.waiver_budget_used || 0), counts,
      starters: (r.starters || []).map((pid, i) => `${slotNames[i] || "?"}: ${pid && pid !== "0" ? pStr(pid) : "empty"}`),
      bench: all.filter(pid => !st8.includes(pid) && !res.includes(pid)).map(pStr),
      ir: res.map(pStr),
    };
  ...
  return {
    ...
    roster, lineup, ir, auto_subs, fa_qbs, fa_top, trending: trending.slice(0, 20), transactions, opponent, qb_depth, teams, stories, news_error, league, env_error: envErrors.length ? envErrors.join("; ") : null, source: "live",
  };
}
