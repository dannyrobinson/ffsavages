...
async function leagueInfo() {
  ...
  leagueCache = {
    ...
    // plays (off = Sleeper's Sept 2024 behaviour, the sub is released). Both players in a pair lock the moment
    // EITHER game kicks off. Which subs a manager has set is NOT in the public API (it lives in the authenticated
...
export async function buildSweep({ force = false } = {}) {
  ...
  // bye weeks, and next week's DEF picture for streaming (projections + schedule for week+1)
  const [BYES, PROJN, SCHEDN] = await Promise.all([
    byeWeeks(st.season).catch(() => ({})), projections(st.season, week + 1, ["DEF"]).catch(() => ({})), schedule(st.season, week + 1).catch(() => ({})),
  ]);
  const enrich = row => {   // projected points + game + bye for any player row
    const pr = PROJ[row.id]; const p = P[row.id];
    row.proj = pr ? pr.proj : 0;
    const g = SCHED[p && p.team]; row.game = gameText(g); row.locked = !!(g && g.status !== "pre_game"); row.date = g ? g.date : null;
  ...
  // players in a pair when EITHER game kicks off, so a sub who plays first only covers a downgrade announced before
  // his own game; the schedule feed gives the day, not the window, so "same day" still needs the kickoff checked.
  ...
  const timing = (s, st) => !s.date || !st.date ? "day unknown" : s.date > st.date ? "later day" : s.date === st.date ? "same day (check the kickoff window)" : "EARLIER day: only covers a downgrade announced before his own kickoff";
  const at_risk = current.filter(c => c.player && !c.player.locked && c.player.flag === "amber" && (c.player.injury || /Practice/.test(c.player.why || ""))).map(c => ({
    starter: c.player.name, id: c.player.id, slot: c.slot, status: c.player.injury || c.player.why, proj: c.player.proj, game: c.player.game, date: c.player.date,
    subs: bench.filter(b => (eligFor[c.slot] || []).includes(b.pos)).map(b => ({ name: b.name, pos: b.pos, proj: b.proj, game: b.game, date: b.date, timing: timing(b, c.player) })).sort((a, b) => b.proj - a.proj),
  }));
  ...
  const defRow = (pid, PR, SCH) => { const p = P[pid]; const pr = PR[pid]; const g = SCH[p && p.team]; return { id: pid, name: p ? p.name : pid, proj: pr ? pr.proj : 0, game: gameText(g) }; };
  const myDef = roster.find(r => r.pos === "DEF");
  const def = {
    mine: myDef ? { name: myDef.name, team: myDef.team, slot: myDef.slot, proj: myDef.proj, game: myDef.game, bye: myDef.bye, next: defRow(myDef.id, PROJN, SCHEDN) } : null,
    fa_this: (fa_top.DEF || []).map(r => ({ name: r.name, proj: r.proj, game: r.game, waivers: r.waivers || null })),
    fa_next: Object.entries(PROJN).filter(([pid, pr]) => pr.proj > 0 && !taken[pid] && P[pid] && P[pid].pos === "DEF")
      .sort((a, b) => b[1].proj - a[1].proj).slice(0, 4).map(([pid]) => defRow(pid, PROJN, SCHEDN)),
  };
  ...
  return {
    ...
    roster, lineup, ir, auto_subs, fa_qbs, fa_top, trending: trending.slice(0, 20), transactions, opponent, qb_depth, league, source: "live",
  };
}
