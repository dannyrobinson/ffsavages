...
export async function buildSweep({ force = false } = {}) {
  ...
  const [BYES, PROJN, SCHEDN, GAMES, GAMESN] = await Promise.all([
    ...
    gamesFor(st.season, week).catch(() => ({})), gamesFor(st.season, week + 1).catch(() => ({})),
  ]);
  ...
  // his own game. Kickoff times come from lib/games.js; when they are missing we fall back to the game day.
  ...
  const timing = (s, st) => {
    if (s.kick_ms && st.kick_ms) return s.kick_ms > st.kick_ms ? `later window (${s.kick_pt}; starter ${st.kick_pt})`
      : s.kick_ms === st.kick_ms ? `same window (${s.kick_pt})`
      : `EARLIER window (${s.kick_pt}; starter ${st.kick_pt}): only covers a downgrade announced before ${s.kick_pt}`;
    return !s.date || !st.date ? "day unknown" : s.date > st.date ? "later day" : s.date === st.date ? "same day (kickoff unknown)" : "EARLIER day: only covers a downgrade announced before his own kickoff";
  };
